/**
 * AlertasNombreRepresentante — Panel de bloqueo por inconsistencia de nombre
 * del representante legal.
 *
 * Renderiza una tarjeta de error por cada documento cuyo nombre de representante
 * no coincide con el campo "Nombres y Apellidos" del formulario. El panel
 * bloquea el avance: no hay botón de descartar. El usuario debe corregir el
 * campo en el formulario o reemplazar el archivo adjunto.
 *
 * SRP : única responsabilidad — mostrar alertas de inconsistencia de nombre de
 *       representante.
...
/**
 * @param {{
 *   alertas: import('../hooks/useAlertasNombreRepresentante').AlertaInconsistenciaNombreRepresentante[],
 * }} props
 */
export default function AlertasNombreRepresentante({ alertas }) {
  ...
  return (
    <div className="alertas-razon-social" role="alert" aria-live="polite">
      ...
      <div className="alertas-razon-social__encabezado">
        ...
        <strong>
          No puedes avanzar: hay {alertas.length === 1 ? 'una inconsistencia' : `${alertas.length} inconsistencias`} de nombre del representante sin resolver
        </strong>
      </div>

      <p className="alertas-razon-social__instruccion">
        Para cada documento marcado, debes <strong>corregir el campo "Nombres y Apellidos"</strong> en
